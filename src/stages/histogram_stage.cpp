...
void HistogramStage::process(Frame& frame) {
    ...
    for (int i = 1; i < histSize; i++) {
        cv::line(hist_image, cv::Point(i - 1, hist_h - cvRound(b_hist.at<float>(i - 1))),
                 cv::Point(i, hist_h - cvRound(b_hist.at<float>(i))), cv::Scalar(255, 0, 0), 1);
        cv::line(hist_image, cv::Point(i - 1, hist_h - cvRound(g_hist.at<float>(i - 1))),
                 cv::Point(i, hist_h - cvRound(g_hist.at<float>(i))), cv::Scalar(0, 255, 0), 1);
